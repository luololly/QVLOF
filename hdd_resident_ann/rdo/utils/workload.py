...
def gen_with_clustering(df, tot_queries, n_clusters=100, noise_level=0.001, random_state=42):
    """
    Cluster the data and randomly sample a specified number of data points
    """
    ...
    print(f"K-means number: {n_clusters}")
    ...
    n_rounds = max(1, tot_queries // 200)
    print(f" {n_rounds} rounds，200 points per round")
    ...
    for round in range(n_rounds):
        ...
        if len(cluster_data) == 0:
            available_clusters = [c for c in range(n_clusters) if len(df_with_clusters[df_with_clusters['cluster'] == c]) > 0]
            selected_cluster = np.random.choice(available_clusters)
            cluster_data = df_with_clusters[df_with_clusters['cluster'] == selected_cluster]

        n_samples = min(200, len(cluster_data))
        sampled_indices = np.random.choice(cluster_data.index, size=200, replace=len(cluster_data) < 200)

        round_samples = df.loc[sampled_indices].copy()

        if noise_level > 0:
            feature_stds = round_samples.std()
            noise = np.random.normal(0, noise_level, round_samples.shape) * feature_stds.values
            round_samples = round_samples + noise

        sampled_data.append(round_samples)

        print(f" {round+1} round complete: sample {len(round_samples)} points in cluster {selected_cluster}")

    final_sampled_data = pd.concat(sampled_data, ignore_index=True)

    if len(final_sampled_data) > tot_queries:
        final_sampled_data = final_sampled_data.sample(n=tot_queries, random_state=random_state)

    print(f"sample points: {len(final_sampled_data)}")
