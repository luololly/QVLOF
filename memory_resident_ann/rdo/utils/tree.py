...
class TreeBuilder:
    def __init__(self, df, df_sample, config, args, k, output_dir, verbose=True):
        ...
        self.out_dir = {}
        for p in self.policy:
            out = "resources/info/%s/%s" % (p, output_dir)
            if not os.path.exists(out):
                os.makedirs(out)
            self.out_dir[p] = out

    def load_by_path(self, path, use_dis=False, sample=False, cols='', init=None, workload=None):
        #use_dis or
        if self.method == "d":
            p = DIS(self.df, self.cfg, self.k, self.args.partial, workload, self.verbose, init,cols)
        else:
            if sample:
    ...
    def compute_optimal_layout(self, queries, path, init,center):
        if self.method == "d":
            dl = DIS(self.df, self.cfg, self.k, self.args.partial, queries, self.verbose, init,center)
        else:
    ...
    def compute_offline_oracle(self, ds, workloads, use_sample=True):
        qfile = self.args.q
        fname = "%s-%s-%d-%s.p" % (ds, qfile, self.k, self.method)
        if self.method == "d":
            dl = DIS(self.df, self.cfg, self.k, self.args.partial, workloads, self.verbose)
        else:
            if use_sample:
    ...
    def get_init_states(self, queries=None):
        ...
        if "oracle" in self.policy:
            ...
            print("----!!!!!!!!!!  DIS")
            init = DIS(self.df, self.cfg, self.k, self.args.partial, None, self.verbose)
